...
export type PlanningProfile = { primarySport: PrimarySport; targetYear: number; weeklyDistanceGoalKm: number; beforeLateShiftAllowed: boolean; afterNightShiftAllowed: boolean; workdayMaxSessionMinutes: number; gymSummerSessions: number; gymWinterSessions: number; indoorCyclingAvailableFrom: string | null; strengthPlan: Record<string, unknown> };

export const defaultPlanningProfile: PlanningProfile = { primarySport: "cycling", targetYear: 2028, weeklyDistanceGoalKm: 125, beforeLateShiftAllowed: true, afterNightShiftAllowed: true, workdayMaxSessionMinutes: 90, gymSummerSessions: 1, gymWinterSessions: 2, indoorCyclingAvailableFrom: "2026-10-01", strengthPlan: {} };
...
export async function getPlanningData(range?: { from: Date; until: Date }): Promise<{ profile: PlanningProfile; events: PlanningEvent[]; ready: boolean }> {
  ...
  const [goalResult, preferencesResult, eventsResult] = await Promise.all([
    ...
    supabase.from("training_preferences").select("primary_sport,before_late_shift_allowed,after_night_shift_allowed,workday_max_session_minutes,gym_summer_sessions,gym_winter_sessions,indoor_cycling_available_from,strength_plan").maybeSingle(),
