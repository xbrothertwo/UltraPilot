...
export async function getTrainingProfile(): Promise<{ profile: TrainingProfile; databaseReady: boolean }> {
  ...
  const { data, error } = await supabase.from("profiles").select("max_heart_rate,resting_heart_rate,ftp_watts,heart_rate_zone_method,custom_heart_rate_boundaries,custom_power_boundaries").maybeSingle();
