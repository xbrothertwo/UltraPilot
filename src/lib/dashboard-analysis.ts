...
export function buildDashboardSummary(activities: Activity[], nutrition: DashboardNutrition[], feedback: DashboardFeedback[], streams: DashboardStream[], profile: TrainingProfile): DashboardSummary {
  const distanceMeters = activities.reduce((sum, activity) => sum + activity.distanceMeters, 0);
  const movingTimeSeconds = activities.reduce((sum, activity) => sum + activity.movingTimeSeconds, 0);
  const elevationGainMeters = activities.reduce((sum, activity) => sum + activity.elevationGainMeters, 0);
  const loads = activities.map((activity) => calculateTrainingLoad(activity.normalizedPower, activity.movingTimeSeconds, profile.ftpWatts)).filter((load): load is NonNullable<typeof load> => load !== null);
  ...
  for (const current of byDate.values()) {
    const activitiesOnDate = activities.filter((activity) => activity.activityDate.slice(0, 10) === current.date);
  ...
  return {
    ...
    movingTimeSeconds,
    elevationGainMeters,
    averageHeartRate: weightedAverage(activities, (activity) => activity.averageHeartRate),
    averagePower: weightedAverage(activities, (activity) => activity.averagePower),
