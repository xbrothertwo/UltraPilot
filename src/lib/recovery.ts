...
export async function getRecoveryData(from: string, until: string): Promise<{ metrics: DailyRecoveryMetric[]; checkins: DailyReadinessCheckin[]; ready: boolean }> {
  ...
  const [metricsResult, checkinsResult] = await Promise.all([
    ...
    supabase.from("daily_readiness_checkins").select("checkin_date,sleep_quality,general_fatigue,leg_fatigue,motivation,pain_or_illness,notes").gte("checkin_date", from).lte("checkin_date", until).order("checkin_date"),
  ]);
  ...
  return {
    ...
    checkins: (checkinsResult.data ?? []).map((row) => ({ date: row.checkin_date, sleepQuality: row.sleep_quality, generalFatigue: row.general_fatigue, legFatigue: row.leg_fatigue, motivation: row.motivation, painOrIllness: row.pain_or_illness, notes: row.notes ?? "" })),
  };
}
