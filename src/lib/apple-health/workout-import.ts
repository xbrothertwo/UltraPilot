...
import { appleHealthSport, appleHealthWorkoutExternalId, isAppleHealthCycling, type AppleHealthWorkoutPayload, type AppleHealthWorkoutSport } from "./workout-parser";
...
export async function persistAppleHealthWorkouts(supabase: SupabaseClient, userId: string, payloads: unknown): Promise<AppleHealthImportResult> {
  ...
  const { data: existing, error: existingError } = await supabase.from("activities").select("external_id").eq("user_id", userId).eq("source", "apple_health_workout").in("external_id", externalIds);
  ...
  for (const workout of workouts) {
    ...
    const { error: activityError } = await supabase.from("activities").insert({
      id: activityId, user_id: userId, sport_type: workout.sportType, activity_date: workout.startTime, title: workout.title,
      distance_meters: workout.distanceMeters, moving_time_seconds: workout.movingTimeSeconds, elapsed_time_seconds: workout.elapsedTimeSeconds,
      elevation_gain_meters: 0, average_speed_kmh: averageSpeedKmh, average_heart_rate: average(heartRates),
      maximum_heart_rate: heartRates.length ? Math.round(Math.max(...heartRates)) : null, average_power: null, normalized_power: null,
      source: "apple_health_workout", external_id: workout.externalId,
    });
    if (activityError) {
    ...
    const stream = workout.heartRateSamples.length ? {
      activity_id: activityId, user_id: userId, stream_type: "heart_rate", source: "apple_watch", unit: "bpm",
      sample_count: workout.heartRateSamples.length, start_time: workout.heartRateSamples[0].timestamp,
      end_time: workout.heartRateSamples.at(-1)!.timestamp, samples: workout.heartRateSamples,
    } : null;
    const [metricsResult, streamResult] = await Promise.all([
      supabase.from("activity_metrics").insert({ activity_id: activityId, user_id: userId, track_point_count: 0, heart_rate_sample_count: workout.heartRateSamples.length, calculation_version: "apple-health-workout-v1", metrics: { activityType: workout.activityType, sourceName: workout.sourceName, endTime: workout.endTime, energyKilocalories: workout.energyKilocalories } }),
