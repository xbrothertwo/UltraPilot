...
export class AppleHealthHeartRateMultiRangeParser {
  private readonly parsers: AppleHealthHeartRateParser[];

  constructor(ranges: { startMilliseconds: number; endMilliseconds: number }[]) {
    this.parsers = ranges.map((range) => new AppleHealthHeartRateParser(range.startMilliseconds, range.endMilliseconds));
  }

  push(chunk: Uint8Array, final = false): void {
    for (const parser of this.parsers) parser.push(chunk, final);
  }

  results(): SensorSample[][] {
    return this.parsers.map((parser) => parser.result());
  }
}
