...
const sleepQualityLabel: Record<number, string> = { 1: "sehr schlecht", 2: "schlecht", 3: "eher schlecht", 4: "mäßig", 5: "durchschnittlich", 6: "solide", 7: "gut", 8: "sehr gut", 9: "hervorragend", 10: "optimal" };
...
export function RecoveryPanel({ readiness, week, editable }: { readiness: ReadinessResult; week: string; editable: boolean }) {
  ...
  return <section className="card mb-4 overflow-hidden"><div className="grid lg:grid-cols-[.8fr_1.2fr]"><div className={`p-5 ${readinessStyle[readiness.status]}`}><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider opacity-60">Tagesform · {readiness.date}</p><h2 className="mt-2 text-xl font-black">{readinessLabel[readiness.status]}</h2></div>{readiness.score !== null && <span className="grid size-14 place-items-center rounded-full bg-white/65 text-xl font-black">{readiness.score}</span>}</div><ul className="mt-4 space-y-1.5 text-sm">{readiness.reasons.map((reason) => <li key={reason}>• {reason}</li>)}</ul>{(metric || checkin?.sleepQuality) && <dl className="mt-5 grid grid-cols-2 gap-3 border-t border-current/10 pt-4 text-sm">{metric && <div><dt className="text-xs opacity-60">Schlafdauer</dt><dd className="font-black">{Math.floor(metric.asleepMinutes / 60)} h {metric.asleepMinutes % 60} min</dd></div>}<div><dt className="text-xs opacity-60">Schlafqualität</dt><dd className="font-black">{checkin?.sleepQuality ? `${checkin.sleepQuality}/10 · ${sleepQualityLabel[checkin.sleepQuality]}` : "–"}</dd></div>{metric && <div><dt className="text-xs opacity-60">Ø HF nachts</dt><dd className="font-black">{metric.sleepingAverageHeartRate ? `${metric.sleepingAverageHeartRate} bpm` : "–"}</dd><p className="text-[.65rem] opacity-60">{metric.heartRateSampleCount} Messpunkte</p></div>}{metric && <div><dt className="text-xs opacity-60">HRV (SDNN)</dt><dd className="font-black">{metric.hrvSdnnMs ? `${metric.hrvSdnnMs} ms` : "–"}</dd></div>}{metric && <div><dt className="text-xs opacity-60">Tief / REM</dt><dd className="font-black">{metric.deepMinutes} / {metric.remMinutes} min</dd></div>}</dl>}</div>
