...
const panelStyles: Record<RideDebriefResult["status"], string> = {
  on_track: "bg-[#0b2347] text-white",
  adjust: "bg-[#f59e67] text-[#3f1f2a]",
  recover: "bg-[#9f3d5b] text-white",
  incomplete: "bg-[#e6efff] text-[#0b2347]",
};

const signalStyles: Record<DebriefTone, string> = {
  good: "bg-emerald-50 text-emerald-950 border-emerald-200",
  info: "bg-blue-50/45 text-slate-900 border-slate-200",
  ...
  return <section id="debrief" className="mt-6 scroll-mt-24">
    <div className={`overflow-hidden rounded-[2rem] p-6 shadow-[0_20px_50px_rgba(16,37,27,.12)] sm:p-8 ${panelStyles[result.status]}`}>
      <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr] xl:items-end">
        ...
        <div className="rounded-2xl border border-current/10 bg-white/10 p-5"><p className="text-xs font-black uppercase tracking-wider opacity-55">Nächster Schritt</p>{result.status === "incomplete" ? <><p className="mt-2 font-black">Zwei Minuten Feedback ergänzen</p><a href="#journal" className="mt-4 inline-flex rounded-xl bg-white px-4 py-2.5 text-sm font-black text-[#0b2347]">Zum Körpergefühl ↓</a></> : (result.nextAction === "easy" || result.nextAction === "recover") && nextWorkout && editable ? <><p className="mt-2 font-black">{nextWorkout.title}</p><p className="mt-1 text-sm opacity-65">{new Intl.DateTimeFormat("de-DE", { weekday: "long", day: "2-digit", month: "2-digit" }).format(new Date(`${nextWorkout.scheduledDate}T12:00:00`))}</p><form action={makeFutureWorkoutEasy.bind(null, activityId)} className="mt-4"><input type="hidden" name="workoutId" value={nextWorkout.id}/><button type="submit" className="rounded-xl bg-white px-4 py-2.5 text-sm font-black text-[#0b2347]">Nächste Fahrt lockern</button></form></> : <><p className="mt-2 font-black">Plan beibehalten</p><p className="mt-1 text-sm opacity-65">Die nächste Einheit muss aktuell nicht verändert werden.</p><Link href="/plan" className="mt-4 inline-flex rounded-xl bg-white px-4 py-2.5 text-sm font-black text-[#0b2347]">Wochenplan öffnen →</Link></>}</div>
      </div>
    </div>
