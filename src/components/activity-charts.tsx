  ...
  return (
    <article className={`card overflow-hidden p-5 ${stream.type === "heart_rate" || stream.type === "altitude" ? "lg:col-span-2" : ""}`}>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="rounded-full bg-[#eef3ef] px-3 py-1 text-xs font-semibold text-[var(--accent-dark)]">{sourceLabels[stream.source]}</span>
      </div>
      <div className="h-56 w-full" role="img" aria-label={`${config.label} über die verstrichene Zeit, Quelle ${sourceLabels[stream.source]}`}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={stream.points} syncId="activity-timeline" syncMethod="value" margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
            ...
            <CartesianGrid stroke="#e5ebe7" strokeDasharray="4 4" vertical={false} />
            <XAxis dataKey="elapsedMinutes" type="number" domain={["dataMin", "dataMax"]} tickFormatter={(value) => elapsedLabel(Number(value))} tick={{ fill: "#66736b", fontSize: 11 }} axisLine={false} tickLine={false} minTickGap={28} />
            <YAxis width={48} domain={["auto", "auto"]} tick={{ fill: "#66736b", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(value) => Number(value).toFixed(config.decimals)} />
            <Tooltip labelFormatter={(value) => elapsedLabel(Number(value))} formatter={(value) => [`${Number(value).toFixed(config.decimals)} ${stream.unit}`, config.label]} contentStyle={{ border: "1px solid #dfe6e1", borderRadius: "12px", boxShadow: "0 8px 24px rgba(35,62,45,.1)" }} />
