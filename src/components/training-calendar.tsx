      ...
      {days.map((date) => {
        ...
        return <section key={date} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const id = event.dataTransfer.getData("text/planned-workout"); if (id) dropWorkout(id, date); }} className={`min-h-44 rounded-2xl border bg-white/75 p-2.5 md:min-h-[32rem] ${date === today ? "border-[var(--accent)] ring-2 ring-[var(--accent)]/10" : "border-[var(--line)]"}`}>
          ...
          <div className="space-y-2">
            ...
            {dayWorkouts.map((item) => { const workout = item.workout; const stateColor = item.effectiveStatus === "completed" ? "border-emerald-300 bg-[#0b2347] text-white" : item.effectiveStatus === "skipped" ? "border-slate-300 bg-slate-100 text-slate-500 opacity-75" : workoutColors[workout.intensity] ?? workoutColors.endurance; return <button key={workout.id} type="button" draggable={item.effectiveStatus === "planned"} onDragStart={(event) => { if (item.effectiveStatus !== "planned") return; event.dataTransfer.setData("text/planned-workout", workout.id); event.dataTransfer.effectAllowed = "move"; }} onClick={() => setEditor({ date, workout })} className={`block w-full rounded-xl border-l-4 p-2.5 text-left shadow-sm ${item.effectiveStatus === "planned" ? "cursor-grab active:cursor-grabbing" : "cursor-pointer"} ${stateColor}`}><p className="text-[.65rem] font-black uppercase tracking-wider opacity-60">{sportLabels[workout.sportType] ?? workout.sportType} · {statusLabels[item.effectiveStatus]} · {intensityLabels[workout.intensity]}</p><p className="mt-0.5 text-[.6rem] font-bold uppercase tracking-wider opacity-45">{sourceLabels[workout.source]}</p><p className={`mt-1 text-sm font-black leading-4 ${item.effectiveStatus === "skipped" ? "line-through" : ""}`}>{workout.title}</p><p className="mt-2 text-[.68rem] font-bold opacity-70">{item.activity ? `${(item.activity.distanceMeters / 1000).toLocaleString("de-DE", { maximumFractionDigits: 1 })} km Ist · ${workout.plannedDistanceKm?.toLocaleString("de-DE", { maximumFractionDigits: 1 }) ?? "–"} km Soll` : workoutMeta(workout, heartRateZones)}</p></button>; })}
