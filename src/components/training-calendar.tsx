...
const kindLabels: Record<string, string> = { work_early: "Frühdienst", work_late: "Spätdienst", work_night: "Nachtdienst", work_day: "Tagdienst", appointment: "Termin", vacation: "Urlaub", free: "Frei", other: "Termin" };
...
function workoutMeta(workout: PlannedWorkout, zones: ZoneDefinition[] | null): string {
  ...
  const target = workout.sportType === "cycling" ? getPlannedHeartRateTarget(zones, workout.intensity) : null;
...
function PlanActualDetails({ item, load }: { item: ReconciledWorkout; load: ActivityLoad | null }) {
  ...
  return <section className="mt-3 rounded-2xl bg-emerald-950 p-4 text-white"><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-wider text-emerald-200">Plan erfüllt</p><Link href={`/activities/${item.activity.id}`} className="text-xs font-bold text-emerald-200 underline">Aktivität öffnen →</Link></div><p className="mt-2 font-black">{item.activity.title}</p><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-emerald-100/60">Distanz Ist / Soll</p><p className="font-bold">{(item.activity.distanceMeters / 1000).toLocaleString("de-DE", { maximumFractionDigits: 1 })} / {item.workout.plannedDistanceKm?.toLocaleString("de-DE", { maximumFractionDigits: 1 }) ?? "–"} km</p>{distance && <p className="text-xs text-emerald-200">{distance}</p>}</div><div><p className="text-xs text-emerald-100/60">Zeit Ist / Soll</p><p className="font-bold">{Math.round(item.activity.movingTimeSeconds / 60)} / {item.workout.plannedDurationMinutes ?? "–"} min</p>{duration && <p className="text-xs text-emerald-200">{duration}</p>}</div></div>{load?.points !== null && load?.points !== undefined && <div className="mt-3 rounded-xl bg-white/10 px-3 py-2 text-sm"><div className="flex items-center justify-between gap-3"><span><strong>{load.points.toLocaleString("de-DE", { maximumFractionDigits: 1 })} UPL</strong> · {comparisonLabel}</span><span className="text-xs text-emerald-100/65">Soll {loadComparison.expectedPoints?.toLocaleString("de-DE", { maximumFractionDigits: 0 }) ?? "–"}</span></div><p className="mt-1 text-xs text-emerald-100/55">{load.detail}</p></div>}</section>;
}
...
function RideStructure({ description }: { description: string | null }) {
  if (!description) return null;
  const steps = description.split("\n").filter(Boolean);
  return <section className="mt-3 rounded-2xl bg-emerald-50 p-4"><p className="text-xs font-black uppercase tracking-wider text-emerald-700">Ablauf der Fahrt</p><ol className="mt-3 space-y-2 text-sm text-emerald-950">{steps.map((step, index) => <li key={`${index}-${step}`} className="flex gap-2"><span className="grid size-5 shrink-0 place-items-center rounded-full bg-emerald-200 text-[.65rem] font-black">{index + 1}</span><span>{step}</span></li>)}</ol></section>;
}

export function TrainingCalendar({ days, week, workouts, events, activities, heartRateZones, readiness, activityLoads }: { days: string[]; week: string; workouts: PlannedWorkout[]; events: PlanningEvent[]; activities: Activity[]; heartRateZones: ZoneDefinition[] | null; readiness: ReadinessResult[]; activityLoads: ActivityLoad[] }) {
      ...
      {days.map((date) => {
        ...
        return <section key={date} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const id = event.dataTransfer.getData("text/planned-workout"); if (id) dropWorkout(id, date); }} className={`min-h-44 rounded-2xl border bg-white/75 p-2.5 md:min-h-[32rem] ${date === today ? "border-[var(--accent)] ring-2 ring-[var(--accent)]/10" : "border-[var(--line)]"}`}>
          ...
          <div className="space-y-2">
            ...
            {dayWorkouts.map((item) => { const workout = item.workout; const stateColor = item.effectiveStatus === "completed" ? "border-emerald-300 bg-emerald-950 text-white" : item.effectiveStatus === "skipped" ? "border-slate-300 bg-slate-100 text-slate-500 opacity-75" : workoutColors[workout.intensity] ?? workoutColors.endurance; return <button key={workout.id} type="button" draggable={item.effectiveStatus === "planned"} onDragStart={(event) => { if (item.effectiveStatus !== "planned") return; event.dataTransfer.setData("text/planned-workout", workout.id); event.dataTransfer.effectAllowed = "move"; }} onClick={() => setEditor({ date, workout })} className={`block w-full rounded-xl border-l-4 p-2.5 text-left shadow-sm ${item.effectiveStatus === "planned" ? "cursor-grab active:cursor-grabbing" : "cursor-pointer"} ${stateColor}`}><p className="text-[.65rem] font-black uppercase tracking-wider opacity-60">{statusLabels[item.effectiveStatus]} · {intensityLabels[workout.intensity]}</p><p className={`mt-1 text-sm font-black leading-4 ${item.effectiveStatus === "skipped" ? "line-through" : ""}`}>{workout.title}</p><p className="mt-2 text-[.68rem] font-bold opacity-70">{item.activity ? `${(item.activity.distanceMeters / 1000).toLocaleString("de-DE", { maximumFractionDigits: 1 })} km Ist · ${workout.plannedDistanceKm?.toLocaleString("de-DE", { maximumFractionDigits: 1 }) ?? "–"} km Soll` : workoutMeta(workout, heartRateZones)}</p></button>; })}
            {dayActivities.map((activity) => <Link key={activity.id} href={`/activities/${activity.id}`} className="block rounded-xl border-l-4 border-[var(--accent)] bg-[var(--ink)] p-2.5 text-white shadow-sm"><p className="text-[.65rem] font-black uppercase tracking-wider text-emerald-200/65">Absolviert</p><p className="mt-1 text-sm font-black leading-4">{activity.title}</p><p className="mt-2 text-[.68rem] font-bold text-emerald-50/65">{(activity.distanceMeters / 1000).toLocaleString("de-DE", { maximumFractionDigits: 1 })} km · {Math.round(activity.movingTimeSeconds / 60)} min</p></Link>)}
          </div>
        </section>;
      })}
    </div>
