...
import { deleteCalendarEvent, deletePlannedWorkout, extendPlannedWorkout, movePlannedWorkout, reduceWorkoutIntensity, saveCalendarEvent, savePlannedWorkout, setPlannedWorkoutLock, setPlannedWorkoutStatus, shortenPlannedWorkout } from "@/app/plan/actions";
...
import { formatHeartRateTarget, formatPaceTarget, formatPowerTarget, getPlannedHeartRateTarget, getPlannedPaceTarget, getZoneTarget, type PaceZone, type ZoneDefinition } from "@/lib/training-zones";
...
const sportLabels: Record<string, string> = { cycling: "Rad", running: "Lauf", strength: "Kraft", volleyball: "Volleyball", mobility: "Mobility", recovery: "Regeneration", other: "Sonstiges" };
...
function HeartRateDetails({ intensity, targetZone, zones }: { intensity: string; targetZone: string | null; zones: ZoneDefinition[] | null }) {
  const target = targetZone ? getZoneTarget(zones, targetZone) : getPlannedHeartRateTarget(zones, intensity);
  return target ? <section className="mt-5 rounded-2xl bg-rose-50 p-4"><p className="text-xs font-black uppercase tracking-wider text-rose-700">Herzfrequenz-Zielbereich</p><p className="mt-1 text-xl font-black text-rose-950">{formatHeartRateTarget(target)}</p><p className="mt-1 text-xs text-rose-900/60">{targetZone ? "Manuell für diese Einheit gewählt." : "Aus deinen persönlichen HF-Zonen in den Einstellungen."}</p></section> : <section className="mt-5 rounded-2xl bg-amber-50 p-4 text-sm text-amber-950">Für einen persönlichen HF-Bereich fehlen Referenzwerte. <Link href="/settings" className="font-bold underline">HF-Zonen einstellen →</Link></section>;
}

function PowerDetails({ targetZone, zones }: { targetZone: string | null; zones: ZoneDefinition[] | null }) {
  if (!targetZone) return null;
  const target = getZoneTarget(zones, targetZone);
  return target ? <section className="mt-3 rounded-2xl bg-indigo-50 p-4"><p className="text-xs font-black uppercase tracking-wider text-indigo-700">Leistungs-Zielbereich</p><p className="mt-1 text-xl font-black text-indigo-950">{formatPowerTarget(target)}</p><p className="mt-1 text-xs text-indigo-900/60">Manuell für diese Einheit gewählt.</p></section> : <section className="mt-3 rounded-2xl bg-amber-50 p-4 text-sm text-amber-950">Für einen persönlichen Leistungsbereich fehlt die FTP. <Link href="/settings" className="font-bold underline">FTP einstellen →</Link></section>;
}

function PaceDetails({ intensity, zones }: { intensity: string; zones: PaceZone[] | null }) {
  const target = getPlannedPaceTarget(zones, intensity);
  return target ? <section className="mt-3 rounded-2xl bg-sky-50 p-4"><p className="text-xs font-black uppercase tracking-wider text-sky-700">Pace-Zielbereich</p><p className="mt-1 text-xl font-black text-sky-950">{formatPaceTarget(target)}</p><p className="mt-1 text-xs text-sky-900/60">Aus deiner Schwellenpace in den Einstellungen.</p></section> : <section className="mt-3 rounded-2xl bg-amber-50 p-4 text-sm text-amber-950">Für einen persönlichen Pace-Bereich fehlt die Schwellenpace. <Link href="/settings" className="font-bold underline">Schwellenpace einstellen →</Link></section>;
}
...
function AutomaticWorkoutDetails({ workout, effectiveStatus, readiness, weekWorkouts }: { workout: PlannedWorkout; effectiveStatus: "planned" | "completed" | "skipped"; readiness: ReadinessResult | undefined; weekWorkouts: PlannedWorkout[] }) {
  if (workout.source !== "automatic") return null;
  const explanations = explainWorkoutPlan(workout, readiness, weekWorkouts);
  const replaceable = effectiveStatus !== "completed" && !workout.locked;
  return <section className="mt-3 rounded-2xl bg-slate-50 p-4">
    <p className="text-xs font-black uppercase tracking-wider text-slate-600">Automatisch geplant</p>
    {explanations.length > 0 && <ul className="mt-2 space-y-1 text-sm text-slate-800">{explanations.map((item) => <li key={item.label}>• {item.text}</li>)}</ul>}
    <p className={`mt-3 text-xs font-bold ${replaceable ? "text-amber-700" : "text-emerald-700"}`}>{replaceable ? "Wird bei erneuter Planung ersetzt, solange sie nicht bearbeitet, gesperrt oder erledigt ist." : workout.locked ? "Gesperrt – wird bei erneuter Planung nicht ersetzt." : "Bereits erledigt – wird bei erneuter Planung nicht ersetzt."}</p>
  </section>;
}

export function TrainingCalendar({ primarySport, days, week, workouts, events, activities, heartRateZones, paceZones, powerZones, readiness, activityLoads }: { primarySport: PrimarySport; days: string[]; week: string; workouts: PlannedWorkout[]; events: PlanningEvent[]; activities: Activity[]; heartRateZones: ZoneDefinition[] | null; paceZones: PaceZone[] | null; powerZones: ZoneDefinition[] | null; readiness: ReadinessResult[]; activityLoads: ActivityLoad[] }) {
  ...
  <div className="mt-1 space-y-1.5 rounded-lg bg-black/5 px-2 py-1.5 text-[.66rem] leading-4 opacity-80">
    ...
    {editor && <div role="dialog" aria-modal="true" aria-label="Training bearbeiten" className="fixed inset-0 z-50 grid place-items-center bg-[var(--ink)]/55 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditor(null); }}><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-[1.5rem] bg-[var(--card)] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="eyebrow">{editor.workout ? "Einheit bearbeiten" : "Einheit planen"}</p><h2 className="mt-2 text-2xl font-black">{new Intl.DateTimeFormat("de-DE", { weekday: "long", day: "numeric", month: "long" }).format(new Date(`${editor.date}T12:00:00`))}</h2></div><button type="button" onClick={() => setEditor(null)} className="grid size-9 place-items-center rounded-full bg-slate-200 text-lg">×</button></div>
      {editorReconciliation && <PlanActualDetails item={editorReconciliation} load={editorReconciliation.activity ? activityLoads.find((load) => load.activityId === editorReconciliation.activity?.id) ?? null : null}/>} {editor.workout && <AutomaticWorkoutDetails workout={editor.workout} effectiveStatus={editorReconciliation?.effectiveStatus ?? "planned"} readiness={readiness.find((item) => item.date === editor.workout!.scheduledDate)} weekWorkouts={weekWorkouts}/>} {editor.workout?.sportType === "strength" && <StrengthWorkoutDetails title={editor.workout.title}/>} {(editor.workout?.sportType === "cycling" || editor.workout?.sportType === "running") && <><HeartRateDetails intensity={editor.workout.intensity} targetZone={editor.workout.targetHeartRateZone} zones={heartRateZones}/>{editor.workout.sportType === "running" && <PaceDetails intensity={editor.workout.intensity} zones={paceZones}/>}{editor.workout.sportType === "cycling" && <PowerDetails targetZone={editor.workout.targetPowerZone} zones={powerZones}/>}<EnduranceStructure description={editor.workout.description} sportType={editor.workout.sportType}/></>}<form action={savePlannedWorkout} className="mt-6 grid gap-4 sm:grid-cols-2"><input type="hidden" name="id" value={editor.workout?.id ?? ""}/><input type="hidden" name="week" value={week}/><label className="text-sm font-bold sm:col-span-2">Name<input name="title" required maxLength={200} defaultValue={editor.workout?.title ?? ""} placeholder={primarySport === "running" ? "z. B. Langer ruhiger Lauf" : "z. B. Lange ruhige Ausfahrt"} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><label className="text-sm font-bold">Datum<input name="scheduledDate" type="date" required defaultValue={editor.date} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><label className="text-sm font-bold">Sport<select name="sportType" defaultValue={editor.workout?.sportType ?? primarySport} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"><option value="cycling">Radfahren</option><option value="running">Laufen</option><option value="strength">Krafttraining</option><option value="volleyball">Volleyball</option><option value="mobility">Mobility</option><option value="recovery">Regeneration</option><option value="other">Sonstiges</option></select></label><label className="text-sm font-bold">Intensität<select name="intensity" defaultValue={editor.workout?.intensity ?? "endurance"} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5">{Object.entries(intensityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm font-bold">Dauer (Minuten)<input name="duration" type="number" min="1" max="1440" defaultValue={editor.workout?.plannedDurationMinutes ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><label className="text-sm font-bold">Bevorzugte Startzeit (optional)<input name="preferredStartTime" type="time" defaultValue={editor.workout?.preferredStartTime ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><label className="text-sm font-bold sm:col-span-2">Distanz (km, optional)<input name="distance" type="number" min="0" max="2000" step="0.1" defaultValue={editor.workout?.plannedDistanceKm ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><label className="text-sm font-bold">Herzfrequenzzone (optional)<select name="targetHeartRateZone" defaultValue={editor.workout?.targetHeartRateZone ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"><option value="">Automatisch aus Intensität</option>{["Z1", "Z2", "Z3", "Z4", "Z5"].map((zone) => <option key={zone} value={zone}>{zone}</option>)}</select></label><label className="text-sm font-bold">Leistungszone (optional)<select name="targetPowerZone" defaultValue={editor.workout?.targetPowerZone ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"><option value="">Nicht gesetzt</option>{["Z1", "Z2", "Z3", "Z4", "Z5", "Z6", "Z7"].map((zone) => <option key={zone} value={zone}>{zone}</option>)}</select></label><label className="text-sm font-bold sm:col-span-2">Beschreibung<textarea name="description" maxLength={3000} rows={3} defaultValue={editor.workout?.description ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><label className="text-sm font-bold sm:col-span-2">Persönliche Notiz<textarea name="personalNote" maxLength={1000} rows={2} placeholder="Nur für dich, z. B. woran du bei dieser Einheit denken willst" defaultValue={editor.workout?.personalNote ?? ""} className="mt-1.5 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2.5"/></label><button type="submit" className="rounded-xl bg-[var(--accent)] px-5 py-3 font-bold text-white sm:col-span-2">{editor.workout ? "Änderungen speichern" : "Einheit einplanen"}</button></form>
      {editor.workout && (editor.workout.plannedDurationMinutes !== null || editor.workout.intensity !== "strength") && <div className="mt-3 flex flex-wrap gap-2">
        {editor.workout.plannedDurationMinutes !== null && <form action={shortenPlannedWorkout}><input type="hidden" name="id" value={editor.workout.id}/><input type="hidden" name="week" value={week}/><button type="submit" className="rounded-xl border border-[var(--line)] bg-white px-3.5 py-2 text-xs font-bold text-[var(--muted)] hover:bg-blue-50/45">Einheit kürzen</button></form>}
        {editor.workout.plannedDurationMinutes !== null && <form action={extendPlannedWorkout}><input type="hidden" name="id" value={editor.workout.id}/><input type="hidden" name="week" value={week}/><button type="submit" className="rounded-xl border border-[var(--line)] bg-white px-3.5 py-2 text-xs font-bold text-[var(--muted)] hover:bg-blue-50/45">Einheit verlängern</button></form>}
        {editor.workout.intensity !== "strength" && <form action={reduceWorkoutIntensity}><input type="hidden" name="id" value={editor.workout.id}/><input type="hidden" name="week" value={week}/><button type="submit" className="rounded-xl border border-[var(--line)] bg-white px-3.5 py-2 text-xs font-bold text-[var(--muted)] hover:bg-blue-50/45">Intensität reduzieren</button></form>}
        <form action={setPlannedWorkoutLock}><input type="hidden" name="id" value={editor.workout.id}/><input type="hidden" name="week" value={week}/><input type="hidden" name="locked" value={editor.workout.locked ? "false" : "true"}/><button type="submit" className={`rounded-xl border px-3.5 py-2 text-xs font-bold ${editor.workout.locked ? "border-amber-200 bg-amber-50 text-amber-900 hover:bg-amber-100" : "border-[var(--line)] bg-white text-[var(--muted)] hover:bg-blue-50/45"}`}>{editor.workout.locked ? "Sperre aufheben" : "Einheit sperren"}</button></form>
      </div>}
