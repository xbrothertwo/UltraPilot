...
export function UploadForm() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
    <form action={formAction} className="card p-6">
      ...
      <div className="mt-5 rounded-xl bg-[#f0f5f1] px-4 py-3 text-xs leading-5 text-[var(--muted)]">Beim Mehrfachimport ordnet UltraPilot die Apple-Health-Werte jeder Fahrt ausschließlich anhand ihres exakten Zeitraums zu. Aktivitäten ohne passende Watch-Werte werden trotzdem importiert.</div>
    ...
    <aside className="card p-6"><p className="eyebrow">Letztes Ergebnis</p>{metrics ? <><dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-6"><Metric label="Distanz" value={formatDistance(metrics.distanceMeters)} /><Metric label="Bewegungszeit" value={formatDuration(metrics.movingTimeSeconds)} /><Metric label="Verstrichene Zeit" value={formatDuration(metrics.elapsedTimeSeconds)} /><Metric label="Ø Geschwindigkeit" value={`${metrics.averageSpeedKmh.toLocaleString("de-DE", { maximumFractionDigits: 1 })} km/h`} /><Metric label="Höhengewinn" value={`${Math.round(metrics.elevationGainMeters).toLocaleString("de-DE")} m`} /><Metric label="Ø Herzfrequenz" value={metrics.averageHeartRate === null ? "Keine Daten" : `${Math.round(metrics.averageHeartRate)} bpm`} /></dl><p className="mt-6 rounded-xl bg-emerald-50 px-4 py-3 text-xs font-medium text-emerald-900">Herzfrequenzquelle: {state.heartRateSource === "apple_watch" ? `Apple Watch (${state.importedHeartRateSamples} Samples)` : state.heartRateSource === "primary" ? "Hauptdatei" : "keine"}</p></> : <p className="mt-4 text-sm leading-6 text-[var(--muted)]">Nach dem Import erscheint hier eine Zusammenfassung der zuletzt erfolgreich verarbeiteten Aktivität.</p>}</aside>
  </div>;
}
...
function FileField({ id, name, title, hint, accept, required = false, multiple = false }: { id: string; name: string; title: string; hint: string; accept: string; required?: boolean; multiple?: boolean }) {
  return <label htmlFor={id} className="mt-5 block rounded-2xl border-2 border-dashed border-[#b8c9be] bg-[#f8faf8] p-5 transition hover:border-[var(--accent)]"><span className="font-semibold">{title}</span><span className="mt-1 block text-xs text-[var(--muted)]">{hint}</span><input id={id} name={name} type="file" accept={accept} required={required} multiple={multiple} className="mt-4 max-w-full text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-800 file:px-4 file:py-2 file:text-white" /></label>;
}
