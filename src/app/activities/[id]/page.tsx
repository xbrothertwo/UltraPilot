  ...
  return (
    <>
      ...
      <section className="card mt-6 p-6">
        ...
        {(trainingLoad || heartRateDrift !== null) && <dl className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{trainingLoad && <><div className="rounded-2xl bg-slate-50 p-4"><dt className="text-sm text-[var(--muted)]">Intensity Factor (IF)</dt><dd className="mt-1 text-2xl font-bold">{trainingLoad.intensityFactor.toLocaleString("de-DE", { maximumFractionDigits: 2 })}</dd></div><div className="rounded-2xl bg-slate-50 p-4"><dt className="text-sm text-[var(--muted)]">Training Stress Score (TSS)</dt><dd className="mt-1 text-2xl font-bold">{Math.round(trainingLoad.tss)}</dd></div></>}{heartRateDrift !== null && <div className="rounded-2xl bg-slate-50 p-4"><dt className="text-sm text-[var(--muted)]">Herzfrequenzdrift</dt><dd className="mt-1 text-2xl font-bold">{heartRateDrift.toLocaleString("de-DE", { maximumFractionDigits: 1 })} %</dd><p className="mt-1 text-xs text-[var(--muted)]">Effizienzverlust Watt / bpm</p></div>}</dl>}
