...
export default async function MissionPage() {
  ...
  return <>
    ...
    <section className="relative overflow-hidden rounded-[2rem] bg-[var(--ink)] p-6 text-white shadow-[0_24px_60px_rgba(16,37,27,.2)] sm:p-9">
      <div className="absolute -right-28 -top-32 size-96 rounded-full border-[60px] border-emerald-300/5"/>
      <div className="relative grid gap-8 xl:grid-cols-[1.25fr_.75fr] xl:items-end"><div><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-300">Supported · Nord nach Süd · Zielkorridor {mission.targetYear}</p><h2 className="mt-3 text-4xl font-black tracking-[-.05em] sm:text-6xl">1.100 km.<br/><span className="text-emerald-200">Schritt für Schritt belastbar.</span></h2><p className="mt-5 max-w-2xl leading-7 text-emerald-50/65">Mission Control misst einzelne Fähigkeiten getrennt. Ein grüner Balken bedeutet nur: Der konkrete, angezeigte Nachweis wurde erbracht – keine Garantie für Rennbereitschaft.</p></div><div className="rounded-2xl border border-white/10 bg-white/[.07] p-5"><p className="text-xs font-black uppercase tracking-wider text-emerald-100/55">Nächster Meilenstein</p><p className="mt-2 text-2xl font-black">{next?.title ?? "Roadmap vollständig"}</p>{next && <><p className="mt-2 text-sm text-emerald-50/60">{next.evidence} · Horizont {next.horizon}</p><div className="mt-4 h-2.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#7bd89e]" style={{ width: `${next.progressPercent}%` }}/></div><p className="mt-2 text-right text-xs font-bold text-emerald-200">{next.progressPercent} % des messbaren Hauptkriteriums</p></>}</div></div>
    </section>
