...
export async function savePlanningProfile(formData: FormData) {
  try {
    ...
    const [{ error: goalError }, { error: preferenceError }] = await Promise.all([
      ...
      supabase.from("training_preferences").upsert({ user_id: user.id, primary_sport: primarySport, running_sessions_per_week: integer(formData, "runningSessions", 1, 7), easy_run_with_cross_training: formData.get("easyRunWithCrossTraining") === "on", before_late_shift_allowed: formData.get("beforeLate") === "on", after_night_shift_allowed: formData.get("afterNight") === "on", workday_max_session_minutes: integer(formData, "workdayMax", 15, 360), gym_summer_sessions: integer(formData, "gymSummer", 0, 7), gym_winter_sessions: integer(formData, "gymWinter", 0, 7), indoor_cycling_available_from: `${indoorYear}-${String(indoorMonth).padStart(2, "0")}-01`, strength_plan: strengthPlan, updated_at: new Date().toISOString() }),
    ]);
...
export async function generateWeeklyPlan(formData: FormData) {
  ...
  try {
    ...
    const crossTrainingActivityDates = new Set(activities.filter((activity) => activity.sportType === "strength" || activity.sportType === "volleyball").map((activity) => new Intl.DateTimeFormat("sv-SE", { timeZone: "Europe/Berlin", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(activity.activityDate))));
    const crossTrainingPlannedDates = new Set([...manual, ...completedAutomatic].filter((workout) => workout.status !== "skipped" && (workout.sportType === "strength" || workout.title.toLowerCase().includes("volleyball"))).map((workout) => workout.scheduledDate));
    const loadProtectedDates = new Set<string>();
    const days = Array.from({ length: 7 }, (_, index) => { const date = new Date(start); date.setDate(date.getDate() + index); const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`; const events = planning.events.filter((event) => new Date(event.startsAt) < new Date(`${key}T23:59:59`) && new Date(event.endsAt) > new Date(`${key}T00:00:00`) && event.eventKind !== "free"); const windows = calculateDailyAvailability(key, events); const recentHighLoad = [1, 2].some((offset) => { const previous = new Date(`${key}T12:00:00Z`); previous.setUTCDate(previous.getUTCDate() - offset); return highLoadDates.has(previous.toISOString().slice(0, 10)); }); if (recentHighLoad) loadProtectedDates.add(key); const measuredReadiness = readinessByDate.get(key); const crossTraining = crossTrainingActivityDates.has(key) || crossTrainingPlannedDates.has(key); const canPair = (workout: { sportType: string; title: string }) => primarySport === "running" && planning.profile.easyRunWithCrossTraining && (workout.sportType === "strength" || workout.title.toLowerCase().includes("volleyball")); return { date: key, availableMinutes: windows.reduce((sum, window) => sum + window.durationMinutes, 0), workday: events.some((event) => event.eventKind.startsWith("work_")), crossTraining, readiness: recentHighLoad && (measuredReadiness === "green" || measuredReadiness === "unknown" || measuredReadiness === undefined) ? "yellow" as const : measuredReadiness, highIntensityAllowed: !recentHighLoad && !events.some((event) => event.eventKind === "work_night"), occupied: key < today || (dashboardAction !== null && key < tomorrow) || activityDates.has(key) || manual.some((workout) => workout.scheduledDate === key && workout.status !== "skipped" && !canPair(workout)) || completedAutomatic.some((workout) => workout.scheduledDate === key && !canPair(workout)) }; });
    const recommendationDays = dashboardAction
      ? days.map((day) => day.date === today ? { ...day, availableMinutes: 0, readiness: "red" as const } : day)
      : days;
    const weeklyRecommendation = recommendWeeklyTarget({ referenceGoalKm: planning.profile.weeklyDistanceGoalKm, days: recommendationDays, recentFourWeekDistanceKm: recentDistanceKm, recentAverageSpeedKmh: averageSpeedKmh, workdayMaxMinutes: planning.profile.workdayMaxSessionMinutes, blockTargetKm: blockWeek?.targetDistanceKm, blockLongRideTargetKm: blockWeek?.longRideTargetKm, blockPhase: blockWeek?.phase });
    ...
    if (remainingDistanceKm === 0) {
      ...
      const generated = generateDeterministicWeek({ primarySport, runningSessionsPerWeek: planning.profile.runningSessionsPerWeek, easyRunWithCrossTraining: planning.profile.easyRunWithCrossTraining, days, weeklyGoalKm: remainingDistanceKm, recentFourWeekDistanceKm: recentDistanceKm, recentAverageSpeedKmh: averageSpeedKmh, workdayMaxMinutes: planning.profile.workdayMaxSessionMinutes, strengthVariants, longRideTargetKm: adaptiveLongRideTargetKm, longRideCovered, tempoSessionTarget: blockWeek?.tempoSessionTarget, preferredCyclingDate: dashboardAction === "shift" ? tomorrow : undefined });
