...
export async function generateWeeklyPlan(formData: FormData) {
  ...
  try {
    ...
    const weeklyGoalKm = blockWeek?.targetDistanceKm ?? planning.profile.weeklyDistanceGoalKm;
    ...
    if (remainingDistanceKm === 0) {
      ...
      const { data: completedGeneration, error } = await supabase.from("training_plan_generations").insert({ user_id: user.id, week_start: week, summary, caution: null, model: "deterministic-v4-blocks", used_ai: false, deterministic_snapshot: { weeklyGoalKm, blockId: trainingBlock?.id ?? null, blockWeek: blockWeek?.weekNumber ?? null, completedDistanceKm, manuallyPlannedDistanceKm, remainingDistanceKm: 0 } }).select("id").single();
