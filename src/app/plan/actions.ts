...
import {
  calculateRemainingWeeklyDistance,
  cyclingDescription,
...
const strengthPlan = {
  cadence: {
    summer: "1× pro Woche, A und B abwechselnd",
    winter: "2× pro Woche, A + B",
  },
  progression:
    "RIR 1–2; zuerst Wiederholungen, dann Gewicht; Grundübungen +2,5–5 kg",
  A: [
    "Back Squat 4×4–6",
    "Bulgarian Split Squat 3×6–8 je Bein",
    "Sitzendes Wadenheben 3×10–12",
    "Pallof Press 3×10–12 je Seite",
    "Dead Bug 3×8–10 je Seite",
    "Side Plank 2×30–45 s je Seite",
  ],
  B: [
    "Romanian Deadlift 4×5–7",
    "Step-up mit Knee Raise 3×8 je Bein",
    "Hip Thrust 3×6–10",
    "Beinbeuger 2×10–12",
    "Stehendes Wadenheben 3×12–15",
    "Forearm Plank 3×30–60 s",
    "Bird Dog 3×8 je Seite",
    "Pallof Hold 2×20–30 s je Seite",
  ],
};
...
support_mode: optionalSupportMode(formData),
        ...
        supabase
          ...
          .upsert({
            ...
            running_sessions_per_week: integer(
              formData,
              "runningSessions",
              1,
              7,
            ...
            easy_run_with_cross_training:
              formData.get("easyRunWithCrossTraining") === "on",
            before_late_shift_allowed: formData.get("beforeLate") === "on",
            after_night_shift_allowed: formData.get("afterNight") === "on",
            workday_max_session_minutes: integer(
              formData,
              "workdayMax",
              15,
              360,
            ),
            gym_summer_sessions: integer(formData, "gymSummer", 0, 7),
            gym_winter_sessions: integer(formData, "gymWinter", 0, 7),
            indoor_cycling_available_from: `${indoorYear}-${String(indoorMonth).padStart(2, "0")}-01`,
...
export async function importCalendarEvents(formData: FormData) {
  ...
  try {
    ...
    if (typeof eventInput !== "string" || typeof mappingInput !== "string")
      throw new Error("Importdaten fehlen.");
    ...
    if (
      ...
      rawEvents.length === 0 ||
      rawEvents.length > 5000
    )
      throw new Error("Ungültige Anzahl von Terminen.");
    const mappings = Object.entries(rawMappings).map(([code, kind]) => {
      if (
        ...
        throw new Error("Eine Code-Zuordnung ist ungültig.");
      return { user_id: user.id, code, event_kind: kind };
    });
    const mappingMap = new Map(
      mappings.map((mapping) => [mapping.code, mapping.event_kind]),
    );
    const events = rawEvents.map((event) => {
      if (
        ...
        throw new Error("Ein Kalendereintrag ist ungültig.");
      ...
      if (
        ...
        throw new Error("Kalenderzeit oder Code-Zuordnung ist ungültig.");
      const eventKey = createHash("sha256")
        .update(
          `${event.title}|${startsAt.toISOString()}|${endsAt.toISOString()}|${Boolean(event.allDay)}`,
        )
        .digest("hex");
      return {
        user_id: user.id,
    ...
    const [{ error: mappingsError }, { error: eventsError }] =
      await Promise.all([
        supabase
          .from("schedule_code_mappings")
          .upsert(mappings, { onConflict: "user_id,code" }),
        supabase
    ...
    if (mappingsError || eventsError)
      throw new Error((mappingsError ?? eventsError)?.message);
    revalidatePath("/plan");
    importedCount = events.length;
  } catch (error) {
    redirect(
      `/plan?error=${encodeURIComponent(error instanceof Error ? error.message : "Kalender konnte nicht importiert werden.")}`,
    );
  }
...
export async function savePlannedWorkout(formData: FormData) {
  ...
  try {
    ...
    if (
      ...
      !sports.includes(sportValue as (typeof sports)[number])
    )
    ...
    const result =
      typeof id === "string" && id
        ? await supabase
            ...
            .eq("id", id)
            .eq("user_id", user.id)
        : await supabase.from("planned_workouts").insert(record);
    if (result.error) throw new Error(result.error.message);
    revalidatePath("/plan");
  } catch (error) {
    destination = planDestination(
      formData,
      `error=${encodeURIComponent(error instanceof Error ? error.message : "Einheit konnte nicht gespeichert werden.")}`,
    );
  }
...
export async function deletePlannedWorkout(formData: FormData) {
  ...
  try {
    ...
    destination = planDestination(
      formData,
      `error=${encodeURIComponent(error instanceof Error ? error.message : "Einheit konnte nicht gelöscht werden.")}`,
    );
  }
  redirect(destination);
}
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Einheit konnte nicht verschoben werden.",
    };
  }
}
...
export async function saveCalendarEvent(formData: FormData) {
  ...
  try {
    ...
    const title = typeof titleValue === "string" ? titleValue.trim() : "";
    if (!title || title.length > 200)
      throw new Error("Bitte gib einen gültigen Titel ein.");
    if (
    ...
    if (typeof id === "string" && id) {
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error || !data)
        throw new Error(error?.message ?? "Termin wurde nicht gefunden.");
    } else {
      const { error } = await supabase
        .from("calendar_events")
        .insert({
          ...changes,
          user_id: user.id,
          event_key: `manual-${crypto.randomUUID()}`,
          source: "manual",
        });
      if (error) throw new Error(error.message);
    }
    ...
    destination = planDestination(
      formData,
      `error=${encodeURIComponent(error instanceof Error ? error.message : "Termin konnte nicht gespeichert werden.")}`,
    );
  }
  redirect(destination);
}
...
export async function deleteCalendarEvent(formData: FormData) {
  ...
  try {
    ...
    destination = planDestination(
      formData,
      `error=${encodeURIComponent(error instanceof Error ? error.message : "Termin konnte nicht gelöscht werden.")}`,
    );
  }
  redirect(destination);
}
...
export async function makeTodayWorkoutEasy(formData: FormData) {
  ...
  try {
    ...
    const { data: workout, error: readError } = await supabase
      .from("planned_workouts")
      .select("id,sport_type,status,planned_duration_minutes")
      .eq("id", id)
      .eq("user_id", user.id)
      .maybeSingle();
    if (readError || !workout)
      throw new Error(readError?.message ?? "Einheit wurde nicht gefunden.");
    if (
      (workout.sport_type !== "cycling" && workout.sport_type !== "running") ||
      workout.status !== "planned"
    )
      throw new Error(
        "Nur eine noch offene Rad- oder Laufeinheit kann gelockert werden.",
      );
    ...
    const { error } = await supabase
      .from("planned_workouts")
      .update({
        title: running ? "Lockerer Dauerlauf" : "Lockere Ausdauerfahrt",
        intensity: "easy",
        description: running
          ? runningDescription("easy", duration)
          : cyclingDescription("easy", duration),
        updated_at: new Date().toISOString(),
      })
      .eq("id", id)
      .eq("user_id", user.id);
  ...
  try {
    ...
    const dashboardTarget =
      dashboardAction && typeof dashboardWorkoutId === "string"
        ...
        : null;
    if (dashboardAction) {
      if (typeof dashboardWorkoutId !== "string" || !dashboardWorkoutId)
        throw new Error("Einheit fehlt.");
      if (
        !dashboardTarget ||
        dashboardTarget.source !== "automatic" ||
        dashboardTarget.status !== "planned" ||
        (dashboardTarget.sportType !== primarySport &&
          dashboardTarget.sportType !== "strength")
      )
        throw new Error(
          "Nur eine offene, automatisch geplante Hauptsport- oder Krafteinheit kann auf diesem Weg umgeplant werden.",
        );
      if (dashboardTarget.scheduledDate !== iso(new Date()))
        throw new Error(
          "Die Schnellaktion ist nur für die heutige Einheit verfügbar.",
        );
      if (
        dashboardAction === "shift" &&
        dashboardTarget.scheduledDate === until
      )
        throw new Error(
          "Am letzten Wochentag kann die Einheit nicht auf morgen verschoben werden.",
        );
    }
    const recentCutoff = start.getTime() - 28 * 86_400_000;
    const recent = activities.filter((activity) => {
      const time = new Date(activity.activityDate).getTime();
      return (
    ...
    const recentDistanceKm = recent.reduce(
    ...
    const completedDistanceKm = completedThisWeek.reduce(
      (sum, activity) => sum + activity.distanceMeters / 1000,
      0,
    );
    ...
    const configuredStrengthSessions = summer
      ? planning.profile.gymSummerSessions
      : planning.profile.gymWinterSessions;
    const currentStrength = manual.filter(
      (workout) =>
        workout.sportType === "strength" && workout.status !== "skipped",
    );
    ...
    const readinessByDate = new Map(
      buildReadinessRange(weekDates, recovery.metrics, recovery.checkins).map(
        (item) => [item.date, item.status],
      ),
    );
    ...
    const activityDates = new Set(
      completedThisWeek.map((activity) =>
    ...
    const crossTrainingActivityDates = new Set(
      activities
        .filter(
          (activity) =>
            activity.sportType === "strength" ||
    ...
    const weeklyRecommendation = recommendWeeklyTarget({
      ...
      recentAverageSpeedKmh: averageSpeedKmh,
      workdayMaxMinutes: planning.profile.workdayMaxSessionMinutes,
      blockTargetKm: blockWeek?.targetDistanceKm,
    ...
    const adaptiveLongRideTargetKm = longRideCovered
      ? undefined
      : Math.min(weeklyRecommendation.longRideTargetKm, remainingDistanceKm);
    const user = await requireUser();
    const supabase = await createClient();
    if (!supabase) throw new Error("Supabase ist nicht verfügbar.");
    const inferredMatches = reconciled.filter(
      (item) =>
        item.activity &&
        (item.workout.linkedActivityId !== item.activity.id ||
    ...
    for (const item of inferredMatches) {
      const { error } = await supabase
        .from("planned_workouts")
        .update({
          linked_activity_id: item.activity!.id,
          status: "completed",
          updated_at: new Date().toISOString(),
        })
        .eq("id", item.workout.id)
        .eq("user_id", user.id);
      if (error)
        throw new Error(
    ...
    if (remainingDistanceKm === 0) {
      ...
      const { data: completedGeneration, error } = await supabase
        ...
        .select("id")
        .single();
      if (error || !completedGeneration)
        throw new Error(
          error?.message ?? "Planlauf konnte nicht gespeichert werden.",
        );
      if (dashboardAction && dashboardTarget) {
        const changes =
          dashboardAction === "shift"
            ? { scheduled_date: tomorrow, updated_at: new Date().toISOString() }
            : {
                status: "skipped",
                linked_activity_id: null,
                updated_at: new Date().toISOString(),
        ...
        if (replaceableAutomatic.length) {
          const { error: removalError } = await supabase
            ...
            .in(
              "id",
              replaceableAutomatic.map((workout) => workout.id),
            );
          if (removalError) {
            await supabase
              .from("training_plan_generations")
              .delete()
              .eq("id", completedGeneration.id);
      ...
      const generated = generateDeterministicWeek({
        ...
        workdayMaxMinutes: planning.profile.workdayMaxSessionMinutes,
        strengthVariants,
        longRideTargetKm: adaptiveLongRideTargetKm,
        longRideCovered,
        tempoSessionTarget: blockWeek?.tempoSessionTarget,
        preferredCyclingDate:
          dashboardAction === "shift" ? tomorrow : undefined,
      });
      ...
      if (
        ...
        throw new Error(
          "Morgen gibt es kein geeignetes freies Trainingsfenster. Der bisherige Plan wurde nicht verändert.",
        );
      const weeklyRecent = recentDistanceKm / 4;
      const increaseWarning =
        weeklyRecent > 0 && weeklyGoalKm > weeklyRecent * 1.1
          ? ` Achtung: Das Ziel liegt mehr als 10 % über deinem Vierwochenschnitt von ${weeklyRecent.toLocaleString("de-DE", { maximumFractionDigits: 1 })} km.`
          : "";
      ...
      const { data: generation, error: generationError } = await supabase
        ...
        .insert({
          ...
          deterministic_snapshot: {
            ...
            fourWeekWeeklyLoad: trainingLoad.fourWeekWeeklyAverage,
            highLoadDates: [...highLoadDates],
            loadProtectedDates: [...loadProtectedDates],
            availableMinutes: days.map((day) => ({
              day: day.date,
      ...
      if (generationError || !generation)
        throw new Error(
          generationError?.message ??
            "Planlauf konnte nicht gespeichert werden.",
        );
      const { error: workoutError } = await supabase
        .from("planned_workouts")
        .insert(
          generated.workouts.map((workout) => ({
            user_id: user.id,
            ...
            planned_distance_km: workout.plannedDistanceKm,
            source: "automatic",
      ...
      if (replaceableAutomatic.length) {
        ...
        if (replacementError) {
          ...
          await supabase
            ...
            .eq("id", generation.id);
          throw new Error(
            `Alter Plan konnte nicht ersetzt werden: ${replacementError.message}`,
          );
        }
      }
    ...
    destination = dashboardAction
      ...
      : planDestination(
          formData,
          `error=${encodeURIComponent(error instanceof Error ? error.message : "Wochenplan konnte nicht erstellt werden.")}`,
        );
  }
  redirect(destination);
}
