...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ week?: string; saved?: string; imported?: string; generated?: string; goal?: string; error?: string }> }) {
  ...
  const actualMinutes = activities.reduce((sum, activity) => sum + activity.movingTimeSeconds / 60, 0);
  const actualKm = activities.reduce((sum, activity) => sum + activity.distanceMeters / 1000, 0);
  ...
  return <>
    ...
    <TrainingCalendar days={days} week={week} workouts={workouts} events={data.events} activities={activities} heartRateZones={getHeartRateZones(trainingProfile)} readiness={readiness} activityLoads={trainingLoad.activities} />
