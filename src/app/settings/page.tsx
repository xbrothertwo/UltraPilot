  ...
  return (
    <>
      ...
      <form action={saveTrainingSettings} className="card max-w-4xl p-6">
        ...
        {(heartRateZones || powerZones) && <section className="mt-8 rounded-2xl bg-slate-50 p-4 text-sm"><h2 className="font-bold">Aktuelle Vorschau</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{heartRateZones && <p><span className="font-semibold">Puls:</span> {heartRateZones.map((zone) => `${zone.name} ${zone.upper ?? "+"}`).join(" · ")}</p>}{powerZones && <p><span className="font-semibold">Leistung:</span> {powerZones.map((zone) => `${zone.name} ${zone.upper ?? "+"}`).join(" · ")}</p>}</div></section>}
