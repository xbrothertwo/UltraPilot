  ...
  return (
    <>
      ...
      <form action={saveTrainingSettings} className="card max-w-4xl p-6">
        <div className="grid gap-8 lg:grid-cols-2">
          <section>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm font-semibold sm:col-span-2">FTP (Watt)<input className={inputClass} name="ftpWatts" type="number" min="50" max="1000" defaultValue={profile.ftpWatts ?? ""} /></label>
            </div>
          </section>
          <section>
        ...
        {(heartRateZones || powerZones) && <section className="mt-8 rounded-2xl bg-blue-50/45 p-4 text-sm"><h2 className="font-bold">Aktuelle Vorschau</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{heartRateZones && <p><span className="font-semibold">Puls:</span> {heartRateZones.map((zone) => `${zone.name} ${zone.upper ?? "+"}`).join(" · ")}</p>}{powerZones && <p><span className="font-semibold">Leistung:</span> {powerZones.map((zone) => `${zone.name} ${zone.upper ?? "+"}`).join(" · ")}</p>}</div></section>}
