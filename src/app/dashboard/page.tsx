...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string; goal?: string }> }) {
  ...
  return <>
    <header className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div><p className="eyebrow">{new Intl.DateTimeFormat("de-DE", { weekday: "long", day: "numeric", month: "long" }).format(today)}</p><h1 className="mt-2 text-4xl font-black tracking-[-.045em] text-[var(--ink)] sm:text-5xl">Heute zählt.</h1><p className="mt-2 max-w-2xl text-[var(--muted)]">Dein Training, deine Erholung und das echte Leben auf einer Seite.</p></div>
      <div className="flex gap-2"><Link href="/plan" className="secondary-button">Wochenplan</Link><Link href="/activities/upload" className="primary-button">+ Importieren</Link></div>
    </header>
    ...
    <section className={`relative overflow-hidden rounded-[2rem] p-6 shadow-[0_24px_60px_rgba(16,37,27,.16)] sm:p-9 ${decisionStyles[decision.level]}`}>
      ...
      <div className="relative grid gap-8 xl:grid-cols-[1.35fr_.65fr] xl:items-end">
        <div><p className="text-xs font-black uppercase tracking-[.18em] opacity-60">{decision.eyebrow}</p><h2 className="mt-3 max-w-3xl text-3xl font-black tracking-[-.04em] sm:text-5xl">{decision.title}</h2><p className="mt-4 max-w-2xl text-base leading-7 opacity-75">{decision.summary}</p>{decision.reasons.length > 0 && <ul className="mt-5 space-y-2 text-sm font-semibold opacity-80">{decision.reasons.map((reason) => <li key={reason} className="flex gap-2"><span aria-hidden>•</span><span>{reason}</span></li>)}</ul>}</div>
