...
import { buildActivityDetailMetrics, heartRateSetupState } from "@/lib/activity-detail";
...
export default async function ActivityPage({ params, searchParams }: ActivityPageProps) {
  ...
  const performanceMetrics = buildActivityDetailMetrics(activity);
  const heartRateState = heartRateSetupState({ hasHeartRateData: activity.averageHeartRate !== null || heartRateSamples.length > 0, hasHeartRateZones: heartRateZones !== null });
  ...
  return (
    <>
      ...
      {matched && !isCycling && <section className="card mt-6 border-emerald-200 p-5 dark:border-emerald-400/20"><p className="text-xs font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-300">✓ Plan-Zuordnung</p><h2 className="mt-2 text-lg font-black">{matched.workout.title}</h2><div className="mt-3 flex flex-wrap gap-2 text-sm text-[var(--muted)]">{matched.workout.plannedDistanceKm !== null && <span>Distanz Ist/Soll {(activity.distanceMeters / 1000).toLocaleString("de-DE", { maximumFractionDigits: 1 })} / {matched.workout.plannedDistanceKm.toLocaleString("de-DE")} km</span>}{matched.workout.plannedDurationMinutes !== null && <span>Zeit Ist/Soll {Math.round(activity.movingTimeSeconds / 60)} / {matched.workout.plannedDurationMinutes} min</span>}</div></section>}
      ...
      {performanceMetrics.length > 0 && <section className="card mt-6 p-6">
        <h2 className="text-lg font-bold">Leistungsdaten</h2>
        <dl className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {performanceMetrics.map((metric) => <div key={metric.label}><dt className="text-sm text-[var(--muted)]">{metric.label}</dt><dd className="mt-1 font-semibold">{metric.value}</dd></div>)}
        </dl>
      </section>}
      <section className="card mt-6 p-6">
        ...
        {(heartRateDistribution || powerDistribution) ? <div className="mt-7 grid gap-8 xl:grid-cols-2">{heartRateDistribution && <ZoneDistribution title="Herzfrequenz" unit="bpm" zones={heartRateDistribution} />}{powerDistribution && <ZoneDistribution title="Leistung" unit="W" zones={powerDistribution} />}</div> : heartRateState === "needs_reference" ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100"><strong>Herzfrequenzdaten erkannt.</strong><p className="mt-1">Für persönliche Trainingszonen und Belastungsberechnung brauchen wir noch deine Referenzwerte. Es werden keine Werte geschätzt.</p><Link href="/settings" className="mt-3 inline-flex font-black underline underline-offset-4">Herzfrequenz-Zonen einrichten</Link></div> : <p className="mt-5 rounded-2xl bg-[var(--surface-raised)] p-4 text-sm leading-6 text-[var(--muted)]">Für diese Aktivität liegen noch keine passenden Zonen-Samples vor. Fehlende Werte werden nicht geschätzt.</p>}
