...
export default async function ProgressPage({ searchParams }: Props) {
  ...
  return <>
    ...
    <div className="mb-4 mt-10"><p className="eyebrow">Training</p><h2 className="mt-2 text-2xl font-black">{summary.primarySport === "running" ? "Laufumfang" : "Radumfang"} und Belastung</h2></div>
    {summary.activityCount === 0 ? <section className="card p-10 text-center"><h2 className="text-xl font-black">Keine {summary.primarySport === "running" ? "Läufe" : "Fahrten"} in diesem Zeitraum</h2><p className="mt-2 text-sm text-[var(--muted)]">Wähle einen längeren Zeitraum oder importiere eine Aktivität.</p></section> : <>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="Aktivitäten" value={String(summary.activityCount)} detail={`letzte ${days} Tage · ${summary.primarySport === "running" ? "Laufen" : "Radfahren"}`} /><StatCard label="Distanz" value={formatDistance(summary.distanceMeters)} detail={summary.primarySport === "running" ? "gesamte Laufstrecke" : "gesamte Radstrecke"} /><StatCard label={summary.primarySport === "running" ? "Ø Pace" : "Ø Geschwindigkeit"} value={summary.averageSpeedKmh === null ? "–" : summary.primarySport === "running" ? formatPace(summary.averageSpeedKmh) : `${summary.averageSpeedKmh.toLocaleString("de-DE", { maximumFractionDigits: 1 })} km/h`} detail="gewichtet nach Bewegungszeit" /><StatCard label="Bewegungszeit" value={formatDuration(summary.movingTimeSeconds)} detail="reine Trainingszeit" /><StatCard label="Höhenmeter" value={`${Math.round(summary.elevationGainMeters).toLocaleString("de-DE")} m`} detail="positiver Anstieg" /></section>
